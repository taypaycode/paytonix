"""
scripts/convert_logos_white.py
Convert client logo assets to white marks on transparent backgrounds.
"""
...
def raster_to_white_transparent(
    source: Path,
    destination: Path,
    *,
    min_channel: int = 18,
    ...
    """
    Convert a raster logo to a white silhouette on a transparent canvas.

    Args:
        source: Input image path.
        destination: Output PNG path.
        min_channel: Keep pixels whose max RGB channel exceeds this value.
        white_only: When True, keep only near-white pixels (for marks on colored backgrounds).
        white_threshold: Minimum RGB value required when white_only is True.
    """
    ...
    for y in range(height):
        for x in range(width):
            ...
            if max(red, green, blue) > min_channel:
...
def main() -> None:
    """Generate white transparent logo assets used by the landing page logo block."""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    raster_to_white_transparent(
        LOGOS_DIR / "icon_Icon-01.png",
        OUTPUT_DIR / "tawkify.png",
        min_channel=8,
    )
    raster_to_white_transparent(
        LOGOS_DIR / "Logo-tritronics.png",
        OUTPUT_DIR / "tritronics.png",
    )
    raster_to_white_transparent(
    ...
    raster_to_white_transparent(
        LOGOS_DIR / "awesomely-logo-mark-squircle.webp",
        OUTPUT_DIR / "awesomely.png",
        min_channel=18,
    )
    write_acrisure_white_svg(OUTPUT_DIR / "acrisure.svg")

    print(f"Wrote white logo assets to {OUTPUT_DIR}")
